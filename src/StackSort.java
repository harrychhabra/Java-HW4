...
public class StackSort {
	...
	// lEFT STACK
    VectorStack<Integer> left = new VectorStack<Integer>(n+1); 
	// right stack
    VectorStack<Integer> right = new VectorStack<Integer>(n+1);
    
    for(int i=0;i < n;i++) {
	// Storing in a varibale
    	int a = data[i];
	// Check if top of left stack is not empty
    	while(left.isEmpty() == false && left.peek() > a) {
    		moveToRight(left,right);
    	}
	// check if top of left is less than top of right
    	while(right.isEmpty() == false && left.isEmpty() == false && right.peek() < a) {
    		moveToLeft(left,right);
    	}
	// push the current element in left stack
    	left.push(a);
    }
	// Moving sorted left stack to right
    while(left.isEmpty() == false) {
    	moveToRight(left,right);
    }
	// Storing in result array using LIFO method
    for(int i=0;i<n;i++) {
    	result[i]=right.pop();
    }
...
//    System.out.println("right: "+right.toString());
    ...
    private static void moveToLeft(VectorStack<Integer> left, VectorStack<Integer> right) {
    	if( right.isEmpty() == true)
    		return;
	// Pop from right and push to left
    	left.push(right.pop());
    }
    private static void moveToRight(VectorStack<Integer> left, VectorStack<Integer> right) {
